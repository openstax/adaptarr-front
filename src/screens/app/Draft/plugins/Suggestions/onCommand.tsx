import { Block, Command, Document, Editor, Inline, InlineProperties, Node, Text } from 'slate'
import { List } from 'immutable'

import { SUGGESTION_TYPES } from './types'

const COMMANDS_TO_HANDLE = [
  'insertText',
  'insertFragment',
  'deleteCharBackward',
  'deleteCharForward',
...
export default function onCommand(command: Command, editor: Editor, next: () => any) {
  ...
  case 'insertInline': {
    const insertInlineProps = command.args[0] as Inline | InlineProperties

    if (!insertInlineProps || SUGGESTION_TYPES.includes(insertInlineProps.type)) return next()

    const insertInline = insertInlineProps instanceof Inline
      ? insertInlineProps
      : Inline.create(insertInlineProps)
...
function getHighestSuggestion(editor: Editor) {
  ...
  if (selection.start.path) {
    highestSuggestion = document.getFurthest(
